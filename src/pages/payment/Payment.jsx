...
const Payment = () => {
  ...
  const livraisonPrice = Number(price.price) * 0.15;

  const total = price.price + protectionPrice + livraisonPrice;

  const options = {
    mode: "payment",
    amount: Math.round(total * 100), // équivaut à 10,99 $
    currency: "eur",
  ...
  return (
    <div className="payment-global-box">
      ...
      <section className="payment-section2 ">
        <Elements stripe={stripePromise} options={options}>
          <CheckoutForm title={title.title} price={price.price} />
        </Elements>
      </section>
    </div>
  );
